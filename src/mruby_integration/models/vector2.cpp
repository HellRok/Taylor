...
auto mrb_Vector2_value(mrb_state* mrb, Vector2* vector) -> mrb_value
{
  mrb_value obj = mrb_obj_value(Data_Wrap_Struct(mrb, Vector2_class, &Vector2_type, vector));

  return obj;
}

auto mrb_Vector2_initialize(mrb_state* mrb, mrb_value self) -> mrb_value
{
  Vector2* vector;
...
auto mrb_Vector2_draw(mrb_state* mrb, mrb_value self) -> mrb_value
{
