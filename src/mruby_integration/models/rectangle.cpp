...
void Rektangle_init(Rektangle* rectangle)
{
...
auto mrb_Rectangle_initialize(mrb_state* mrb, mrb_value self) -> mrb_value
{
  Rektangle* rectangle;
  ...
  mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@colour"), mrb_Color_value(mrb, rectangle->colour));

  if (!mrb_undef_p(kw_values[5])) {
    rectangle->outline = static_cast<struct Color*> DATA_PTR(kw_values[5]);
    add_reference(rectangle->outline);
    mrb_iv_set(
      mrb, self, mrb_intern_cstr(mrb, "@outline"), mrb_Color_value(mrb, rectangle->outline));
  } else {
...
mrb_attr_accessor_with_klasses_and_attrs(mrb, self, float, f, Rectangle, Rektangle, x, rectangle.x);
mrb_attr_accessor_with_klasses_and_attrs(mrb, self, float, f, Rectangle, Rektangle, y, rectangle.y);
...
auto mrb_Rectangle_set_thickness(mrb_state* mrb, mrb_value self) -> mrb_value
{
...
auto mrb_Rectangle_set_roundness(mrb_state* mrb, mrb_value self) -> mrb_value
{
  mrb_get_self(mrb, self, Rektangle, rectangle);
  mrb_float value;
  mrb_get_args(mrb, "f", &value);
...
auto mrb_Rectangle_set_segments(mrb_state* mrb, mrb_value self) -> mrb_value
{
  mrb_get_self(mrb, self, Rektangle, rectangle);
  mrb_int value;
...
auto mrb_Rectangle_draw(mrb_state* mrb, mrb_value self) -> mrb_value
{
  mrb_get_self(mrb, self, Rektangle, rectangle);
  ...
  if (rectangle->roundness == 0) {
    ...
    if (rectangle->outline != nullptr) {
      DrawRectangleLinesEx(rectangle->rectangle, rectangle->thickness, *rectangle->outline);
    }
  } else {
    DrawRectangleRounded(
      rectangle->rectangle, rectangle->roundness, rectangle->segments, *rectangle->colour);
    if (rectangle->outline != nullptr) {
...
auto mrb_Rectangle_begin_scissoring(mrb_state* mrb, mrb_value self) -> mrb_value
{
  mrb_get_self(mrb, self, Rektangle, rectangle);
...
auto mrb_Rectangle_end_scissoring(mrb_state*, mrb_value) -> mrb_value
{
  EndScissorMode();
  return mrb_nil_value();
}
...
void append_models_Rectangle(mrb_state* mrb)
{
  ...
  mrb_define_method(mrb, Rectangle_class, "draw", mrb_Rectangle_draw, MRB_ARGS_NONE());
  mrb_define_method(
    mrb, Rectangle_class, "begin_scissoring", mrb_Rectangle_begin_scissoring, MRB_ARGS_NONE());
  mrb_define_method(
    mrb, Rectangle_class, "end_scissoring", mrb_Rectangle_end_scissoring, MRB_ARGS_NONE());
