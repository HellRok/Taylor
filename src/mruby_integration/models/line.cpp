...
auto mrb_Line_initialize(mrb_state* mrb, mrb_value self) -> mrb_value
{
  Line* line;
  ...
  if (!mrb_undef_p(kw_values[0])) {
    ...
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@start"), mrb_Vector2_value(mrb, line->start));
  } else {
    line->start = nullptr;
  }
  ...
  if (!mrb_undef_p(kw_values[1])) {
    ...
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@end"), mrb_Vector2_value(mrb, line->end));
  } else {
    line->end = nullptr;
  }
  ...
  if (!mrb_undef_p(kw_values[2])) {
    ...
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@colour"), mrb_Color_value(mrb, line->colour));
  } else {
    line->colour = nullptr;
  }
...
auto mrb_Line_set_thickness(mrb_state* mrb, mrb_value self) -> mrb_value
{
...
auto mrb_Line_draw(mrb_state* mrb, mrb_value self) -> mrb_value
{
  mrb_get_self(mrb, self, Line, line);
...
void append_models_Line(mrb_state* mrb)
{
