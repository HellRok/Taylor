...
auto mrb_Circle_initialize(mrb_state* mrb, mrb_value self) -> mrb_value
{
  ...
  mrb_sym kw_names[] = { mrb_intern_lit(mrb, "x"),       mrb_intern_lit(mrb, "y"),
  ...
  mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@colour"), mrb_Color_value(mrb, circle->colour));

  if (!mrb_undef_p(kw_values[4])) {
    circle->outline = static_cast<struct Color*> DATA_PTR(kw_values[4]);
    add_reference(circle->outline);
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@outline"), mrb_Color_value(mrb, circle->outline));
  } else {
  ...
  if (!mrb_undef_p(kw_values[6])) {
    ...
    mrb_iv_set(
      mrb, self, mrb_intern_cstr(mrb, "@gradient"), mrb_Color_value(mrb, circle->gradient));
  } else {
...
auto mrb_Circle_set_radius(mrb_state* mrb, mrb_value self) -> mrb_value
{
...
auto mrb_Circle_set_thickness(mrb_state* mrb, mrb_value self) -> mrb_value
{
  mrb_get_self(mrb, self, Circle, circle);
  mrb_float value;
  mrb_get_args(mrb, "f", &value);
...
auto mrb_Circle_draw(mrb_state* mrb, mrb_value self) -> mrb_value
{
  mrb_get_self(mrb, self, Circle, circle);
  ...
  if (circle->outline != nullptr) {
    ...
    DrawPolyLinesEx(Vector2{ static_cast<float>(circle->x), static_cast<float>(circle->y) },
                    60,
                    circle->radius,
                    0,
                    circle->thickness,
...
void append_models_Circle(mrb_state* mrb)
{
  ...
  mrb_define_method(mrb, Circle_class, "colour=", mrb_Circle_set_colour, MRB_ARGS_REQ(1));
  mrb_define_method(mrb, Circle_class, "outline=", mrb_Circle_set_outline, MRB_ARGS_REQ(1));
  mrb_define_method(mrb, Circle_class, "gradient=", mrb_Circle_set_gradient, MRB_ARGS_REQ(1));
  mrb_define_method(mrb, Circle_class, "draw", mrb_Circle_draw, MRB_ARGS_NONE());
