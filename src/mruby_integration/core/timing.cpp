...
mrb_value mrb_get_time(mrb_state *mrb, mrb_value) {
  return mrb_float_value(mrb, GetTime());
}
void append_core_timing(mrb_state *mrb) {
  ...
  mrb_define_method(mrb, mrb->kernel_module, "get_time", mrb_get_time, MRB_ARGS_NONE());
}
