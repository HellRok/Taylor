...
auto
mrb_get_world_to_screen2D(mrb_state* mrb, mrb_value) -> mrb_value
{
  Vector2* vector;
  Camera2D* camera;
  mrb_get_args(mrb, "dd", &vector, &Vector2_type, &camera, &Camera2D_type);

  auto* new_vector = static_cast<Vector2*>(malloc(sizeof(Vector2)));
...
auto
mrb_get_screen_to_world2D(mrb_state* mrb, mrb_value) -> mrb_value
{
  Vector2* vector;
  Camera2D* camera;
  mrb_get_args(mrb, "dd", &vector, &Vector2_type, &camera, &Camera2D_type);
