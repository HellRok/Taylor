...
Rectangle gui_font_rectangle{ 0, 0, 0, 0 };
Image image{ 0, 0, 0, 0, 0 };
CharInfo gui_font_char_info{ 0, 0, 0, 0, image};
Texture2D texture{ 0, 0, 0, 0, 0 };
static Font guiFont = { 0, 0, 0,
  texture,
  &gui_font_rectangle,
  &gui_font_char_info
...
Rectangle GuiScrollPanel(Rectangle bounds, Rectangle content, Vector2 *scroll)
{
    ...
    if (hasHorizontalScrollBar && hasVerticalScrollBar)
    {
        ...
        Rectangle corner = { (float)horizontalScrollBar.x + (float)horizontalScrollBar.width + 2, (float)verticalScrollBar.y + (float)verticalScrollBar.height + 2, (float)horizontalScrollBarWidth - 4, (float)verticalScrollBarWidth - 4 };
...
bool GuiTextBoxMulti(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    Rectangle cursor = { 0, 0, 1, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) + 2.0f };
...
int GuiScrollBar(Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    // Arrow buttons [<] [>] [∧] [∨]
    Rectangle arrowUpLeft = { 0, 0, 0, 0 };
    Rectangle arrowDownRight = { 0, 0, 0, 0 };

    // Actual area of the scrollbar excluding the arrow buttons
    Rectangle scrollbar = { 0, 0, 0, 0 };

    // Slider bar that moves     --[///]-----
    Rectangle slider = { 0, 0, 0, 0 };
...
Color GuiColorPanelEx(Rectangle bounds, Color color, float hue)
{
    ...
    if (state != GUI_STATE_DISABLED)
    {
        ...
        Rectangle selector = { pickerSelector.x - (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, pickerSelector.y - (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE), (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE) };
...
int GuiMessageBox(Rectangle bounds, const char *title, const char *message, const char *buttons)
{
    ...
    Rectangle buttonBounds = { 0, 0, 0, 0 };
...
int GuiTextInputBox(Rectangle bounds, const char *title, const char *message, const char *buttons, char *text)
{
    ...
    Rectangle textBoxBounds = { 0, 0, 0, 0 };
            ...
            while (!feof(rgsFile))
            {
                switch (buffer[0])
                {
                    ...
                    case 'f':
                    {
                        ...
                        Image image{ 0, 0, 0, 0, 0 };
                        CharInfo char_info{ 0, 0, 0, 0, image};
                        Texture2D texture { 0, 0, 0, 0, 0 };
                        Font font = { 0, 0, 0,
                          texture,
                          &rectangle,
