...
#define attr_setter_int(mrb, self, klass_type, klass, attr, ivar)                                  \
  {                                                                                                \
    ...
    mrb_assert(data != nullptr);                                                                   \
                                                                                                   \
    data->attr = attr;                                                                             \
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@" #ivar), mrb_int_value(mrb, attr));              \
                                                                                                   \
    return mrb_int_value(mrb, data->attr);                                                         \
  }
...
#define attr_setter_float(mrb, self, klass_type, klass, attr, ivar)                                \
  {                                                                                                \
    ...
    mrb_assert(data != nullptr);                                                                   \
                                                                                                   \
    data->attr = attr;                                                                             \
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@" #ivar), mrb_float_value(mrb, attr));            \
                                                                                                   \
...
#define attr_setter_bool(mrb, self, klass_type, klass, attr, ivar)                                 \
  {                                                                                                \
    ...
    mrb_assert(data != nullptr);                                                                   \
                                                                                                   \
    data->attr = attr;                                                                             \
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@" #ivar), mrb_bool_value(attr));                  \
                                                                                                   \
    return mrb_bool_value(data->attr);                                                             \
  }
...
#define ivar_attr_int(mrb, self, value, attr)                                                      \
  {                                                                                                \
    value = attr;                                                                                  \
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@" #attr), mrb_int_value(mrb, value));             \
  }

#define ivar_attr_float(mrb, self, value, attr)                                                    \
  {                                                                                                \
    value = attr;                                                                                  \
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@" #attr), mrb_float_value(mrb, value));           \
  }

#define ivar_attr_bool(mrb, self, value, attr)                                                     \
  {                                                                                                \
    value = attr;                                                                                  \
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@" #attr), mrb_bool_value(value));                 \
  }

#define ivar_attr_vector2(mrb, self, obj_value, attr)                                              \
  {                                                                                                \
    mrb_value obj =                                                                                \
      mrb_obj_value(Data_Wrap_Struct(mrb, Vector2_class, &Vector2_type, &obj_value));              \
    obj_value.x = attr->x;                                                                         \
    obj_value.y = attr->y;                                                                         \
    mrb_iv_set(mrb, obj, mrb_intern_cstr(mrb, "@x"), mrb_float_value(mrb, attr->x));               \
    mrb_iv_set(mrb, obj, mrb_intern_cstr(mrb, "@y"), mrb_float_value(mrb, attr->y));               \
                                                                                                   \
...
#define ivar_attr_texture2d(mrb, self, obj_value, attr)                                            \
  {                                                                                                \
    mrb_value obj =                                                                                \
      mrb_obj_value(Data_Wrap_Struct(mrb, Texture2D_class, &Texture2D_type, &obj_value));          \
                                                                                                   \
    mrb_iv_set(mrb, obj, mrb_intern_cstr(mrb, "@id"), mrb_float_value(mrb, attr->id));             \
    mrb_iv_set(mrb, obj, mrb_intern_cstr(mrb, "@width"), mrb_float_value(mrb, attr->width));       \
    mrb_iv_set(mrb, obj, mrb_intern_cstr(mrb, "@height"), mrb_float_value(mrb, attr->height));     \
    mrb_iv_set(mrb, obj, mrb_intern_cstr(mrb, "@mipmaps"), mrb_float_value(mrb, attr->mipmaps));   \
    mrb_iv_set(mrb, obj, mrb_intern_cstr(mrb, "@format"), mrb_float_value(mrb, attr->format));     \
                                                                                                   \
    mrb_iv_set(mrb, self, mrb_intern_cstr(mrb, "@" #attr), obj);                                   \
  }
...
#define mrb_get_self(mrb, self, klass, data)                                                       \
  klass* data;                                                                                     \
...
#define mrb_attr_reader_with_klasses_and_attrs(                                                    \
  ...
  auto mrb_##ruby_klass##_##ruby_attr(mrb_state* mrb, mrb_value self) -> mrb_value                 \
  {                                                                                                \
...
#define mrb_attr_reader_with_klasses(mrb, self, type, ruby_klass, cpp_klass, attr)                 \
  mrb_attr_reader_with_klasses_and_attrs(mrb, self, type, ruby_klass, cpp_klass, attr, attr);
...
#define mrb_attr_writer_with_klasses_and_attrs(                                                    \
  ...
  auto mrb_##ruby_klass##_set_##ruby_attr(mrb_state* mrb, mrb_value self) -> mrb_value             \
  {                                                                                                \
    mrb_get_self(mrb, self, cpp_klass, data);                                                      \
    mrb_##type ruby_attr;                                                                          \
...
#define mrb_attr_writer_with_klasses(mrb, self, type, mrb_arg, ruby_klass, cpp_klass, attr)        \
...
#define mrb_attr_writer_struct(mrb, self, ruby_type, cpp_type, attr, klass)                        \
  ...
  {                                                                                                \
    ...
    if (value) {                                                                                   \
      ...
      mrb_iv_set(                                                                                  \
        mrb, self, mrb_intern_cstr(mrb, "@" #attr), mrb_##klass##_value(mrb, data->attr));         \
    } else {                                                                                       \
...
#define mrb_attr_accessor_with_klasses(mrb, self, type, mrb_arg, ruby_klass, cpp_klass, attr)      \
  mrb_attr_reader_with_klasses(mrb, self, type, ruby_klass, cpp_klass, attr);                      \
  mrb_attr_writer_with_klasses(mrb, self, type, mrb_arg, ruby_klass, cpp_klass, attr);
...
#define mrb_attr_reader_define(mrb, klass, attr)                                                   \
  mrb_define_method(mrb, klass##_class, #attr, mrb_##klass##_##attr, MRB_ARGS_NONE());

#define mrb_attr_writer_define(mrb, klass, attr)                                                   \
  mrb_define_method(mrb, klass##_class, #attr "=", mrb_##klass##_set_##attr, MRB_ARGS_REQ(1));
