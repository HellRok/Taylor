...
This has been removed in favour of {Window.minimum_size=}, it also now takes a
{Vector2} instead of `x` and `y` as integers.
...
```ruby
Window.minimum_size = Vector2[x, y]
```
...
This has been removed in favour of {Window.size=}, it also now takes a
...
```ruby
Window.minimum_size = Vector2[width, height]
```
