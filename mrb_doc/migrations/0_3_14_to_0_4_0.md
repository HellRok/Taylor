# Migrating from 0.3.14 to 0.4.0

## MTest

This has been dropped in favour of [neospec](https://github.com/HellRok/neospec)
...
```ruby
camera = Camera2D.new(Vector2.new(x: 0, y: 0), Vector2.new(x: 0, y: 0), 0, 1)
```
...
This has been removed in favour of {Shader.valid?}
...
{Vector2#initialize} uses keyword arguments.
...
This has been replaced by {Font#valid?}.
...
```ruby
font.draw("hello", position: Vector2.new(x: 16, y: 42))
```
...
Previously to draw text using the default font `draw_text` would've been used.
```ruby
draw_text("hello", 16, 42, 16)
```

This behavior has been removed and now `Font#default` is used to access the default font.
```ruby
Font.default.draw("hello", x: 16, y: 42, size: 16)
````
